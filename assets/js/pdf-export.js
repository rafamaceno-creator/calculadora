...
function collectReportItems() {
  const cards = Array.from(document.querySelectorAll("#ledgerResults .ledgerCard"));
  const productNameRaw = document.querySelector("#calcName")?.value?.trim() || "";
  const productName = productNameRaw || "Produto sem nome";

  return cards.map((card) => ({
    productName,
    marketplace: card.dataset.reportName || "Marketplace",
    suggestedPrice: card.dataset.reportPrice || "—",
    received: card.dataset.reportReceived || "—",
    profit: card.dataset.reportProfit || "—",
    commission: card.dataset.reportCommission || "—"
  }));
}

function getReportMode() {
  const mode = document.querySelector('input[name="calcMode"]:checked')?.value;
  return mode === "real" ? "real" : "ideal";
}
...
function getReportMarkup() {
  ...
  const isReal = getReportMode() === "real";
  ...
  return `
    <section class="print-report" style="font-family:Inter,Arial,sans-serif;color:#0f172a;padding:20px;">
      <header style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;border-bottom:2px solid #e2e8f0;padding-bottom:12px;margin-bottom:14px;">
        <div>
          <div style="font-size:12px;letter-spacing:.08em;color:#334155;font-weight:700;text-transform:uppercase;">${isReal ? "Lucro real por canal" : "Preço sugerido por canal"}</div>
      ...
      <section class="report-card" style="margin-top:14px;padding:14px;border:1px solid #d8dee8;border-radius:12px;break-inside:avoid;page-break-inside:avoid;">
        ...
        <div style="display:grid;grid-template-columns:1fr auto;gap:7px 12px;font-size:13px;">
          <span>${isReal ? "Preço de venda" : "Preço sugerido"}</span><strong>${escapeHTML(item.suggestedPrice)}</strong>
          <span>Você recebe</span><strong>${escapeHTML(item.received)}</strong>
          <span>Comissão do canal</span><strong>${escapeHTML(item.commission)}</strong>
...
window.getReportMarkup = getReportMarkup;
